...
# RUN git config --global http.proxy http://192.168.0.104:7890 && \
#     git config --global https.proxy http://192.168.0.104:7890
RUN git clone https://github.com/luanshaotong/text-generation-webui /src
...
RUN head -n -3 /app/requirements.txt > /app/requirements2.txt
RUN --mount=type=cache,target=/root/.cache/pip pip3 install -r /app/requirements2.txt
# Install extensions
